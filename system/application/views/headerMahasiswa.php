<div id="header">
    <div id="header-content" class="container_12">
        <!-- Logo Web -->
        <!--<a id="header-logo" href="#"><img src="<?=base_url()?>assets/skins/bento/header-logo.png" width="46" height="26" alt="Header Logo" /></a>-->
        <a id="header-logo" href="<?php echo base_url();?>"><img src="<?=base_url()?>assets/images/images_book logo2.gif" width="86" height="30" alt="Header Logo" /></a>
        <ul id="global-navigation">
            <li id="item-panduan"><a href="#">Panduan TA</a></li>
            <li><a href="<?php echo site_url('topik/lihatTopik')?>">Topik TA</a></li>
            <li><a href="<?php echo site_url('progres/tugasakhir')?>">Pencarian TA</a></li>
            <li id="item-proposal"><a href="#">Proposal TA</a></li>
            <li id="item-cetakcover"><a href="#">Cetak Cover</a></li>
            <li id="item-sidang"><a href="#">Sidang</a></li>
            <li><a href="<?php echo site_url('progres/progresSaya')?>">Progres TA</a></li>
            <li><a href="<?php echo site_url('berita/lihatBerita')?>">Berita</a></li>
            <li id="item-lamastudi"><a href="#">Lama Pengerjaan TA</a></li>
      </ul>
    </div>
</div>
